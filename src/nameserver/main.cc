 ...
 * Mapping     : CORBA C
 * 
 * Generated by IDL4 1.1.0 (roadrunner) on 09/07/2012 21:12
...
{
  printf("[NAMESERVER] Got register message for \"%s\" from thread %lx\n", path, _caller.raw);
...

{
  ...
  while (1)
	...
	IF_LOGGING_LogMessage((CORBA_Object)loggerid, "[NAMESERVER] idl4_msgbuf_sync", &env);

          idl4_reply_and_wait(&partner, &msgtag, &msgbuf, &cnt);
	IF_LOGGING_LogMessage((CORBA_Object)loggerid, "[NAMESERVER] idl4_reply_and_wait", &env);
...
int main(void)
{
    ...
    IF_LOGGING_LogMessage((CORBA_Object)loggerid, "[NAMESERVER] Starting", &env);
